...
export const ChartView = (props: Props = { data: {} }) => {
  ...
  const allFillings = currentDateDateValues.reduce((acc: number, value: number) => acc += value, 0);
  ...
  const maximumNumberOfFillsItemId = Object.keys(currentDateDate)[Object.values(currentDateDate).findIndex(el => el == maximumNumberOfFills)];
  let maximumNumberOfFillsInPercentage: any = (100 * maximumNumberOfFills) / data.allDepartments.find((department: any) => department.departmentId == maximumNumberOfFillsItemId).value;
  ...
  maximumNumberOfFillsInPercentage = Math.max(...getDepartmentNamesFromCurrentDate().map((item: any) => item.value));
  maximumNumberOfFillsInPercentage = parseFloat(maximumNumberOfFillsInPercentage).toFixed(2);

  const sumOfFilling: any = getDepartmentNamesFromCurrentDate()
    .map((item: any) => item.value)
    .reduce((acc: number, val) => acc += val, 0);
  averageNumberOfFills = parseFloat((sumOfFilling / getDepartmentNamesFromCurrentDate().length).toString()).toFixed(2);
  ...
  return (
    <>
      <SimpleStats allFillings={ allFillingsInPercentage }
